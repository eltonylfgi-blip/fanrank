...
| Role | Can do |
| --- | --- |
| Owner | Heart ideas, invite/revoke admins and contributors. |
| Admin | Heart ideas, invite/revoke contributors. |
| Contributor | Heart ideas only. |
...
- The public interface exposes only the aggregate heart count. It never exposes which individual member hearted an idea.

## Ranking signal

The deployed ranking display is intentionally bounded:

```text
display score = AI base + min(fan votes × 2, 20) + min(distinct team hearts × 5, 15)
```

Team interest is evidence, not a 100-point override and not a guarantee that the idea ranks first. It is only available for a verified profile. A future learned company criterion must run in shadow mode first, with logged add/remove signals, shown-but-not-hearted exposures, and rank snapshots; it must not change public ordering until it has a measured validation result.

The legacy `owner_pick` field is pinned to `false` and is excluded from every browser ordering path. It cannot turn a team heart into an automatic featured or first-ranked idea.
...
- The `fr_team_interests` and `fr_profile_invites` tables have RLS without direct client policies by design: only the guarded RPCs operate on them.
